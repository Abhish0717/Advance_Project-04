	...
	<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
		...
		<div class="card-header text-white border-0 py-3 px-4"
			...
			<h5 class="mb-0 fw-bold">
				...
				<%=bean.getId() > 0 ? ms.get("subject.edit") : ms.get("subject.add")%>
			</h5>
		</div>
			...
			<form action="<%=ORSView.SUBJECT_CTL%>" method="POST">
				...
				<div class="mb-3">
					<label class="form-label fw-semibold"><%=ms.get("name.title")%>
						...
						placeholder="<%=ms.get("subject.placeholder")%>"
				...
				<div class="mb-3">
					<label class="form-label fw-semibold"><%=ms.get("description.title") %> <span
						class="text-danger">*</span></label> <input type="text" name="description" placeholder="<%=ms.get("description.placeholder")%>"
				...
				<div class="mb-4">
					<label class="form-label fw-semibold"><%=ms.get("course.title") %> <span
						class="text-danger">*</span></label> <select name="courseId"
						class="form-select">
						<option value="">------------------------------<%=ms.get("course.select") %>------------------------------</option>
						<%
				...
				<div class="d-flex gap-2 pt-2 border-top">
					...
					%>
					<a href="SubjectCtl" class="btn btn-danger ms-auto"> <i
