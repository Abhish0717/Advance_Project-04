<%@page import="com.sunilos.p4.ctl.ApiListCtl"%>
<%@page import="com.sunilos.p4.ctl.BaseCtl"%>
<%@page import="com.sunilos.p4.ctl.ORSView"%>
<%@page import="com.sunilos.p4.bean.ApiBean"%>
	...
	<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
		<div
			...
			<a href="<%=ORSView.API_CTL%>"
				...
				class="bi bi-plus-circle me-1"></i> Add API Info
			</a>
		</div>
	</div>
	<form action="<%=ORSView.API_LIST_CTL%>" method="POST">
			...
			<table class="table table-hover align-middle mb-0">
				...
				<tbody>
					...
					<tr>
						...
						<td class="fw-semibold"><%=bean.getApiName()%></td>
						<td><%=bean.getVersion()%></td>
						<td><%=bean.getEndPoint()%></td>
						<td><%=bean.getStatus()%></td>
						<td><a href="ApiCtl?id=<%=bean.getId()%>"
