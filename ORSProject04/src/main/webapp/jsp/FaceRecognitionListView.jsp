...
<%@page import="com.sunilos.p4.ctl.FaceRecognitionListCtl"%>
	...
	<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
		<div
			...
			<h5 class="mb-0 fw-bold">
				<i class="bi bi-person-bounding-box me-2"></i>Face Recognition List
			</h5>

			<div class="d-flex gap-2">
				<a href="<%=ORSView.FACE_RECOGNITION_REPORT_CTL%>" target="_blank"
					class="btn btn-sm btn-warning fw-semibold"> <i
					class="bi bi-file-earmark-pdf me-1"></i> <%=ms.get("print.pdf")%>
				</a> <a href="<%=ORSView.FACE_RECOGNITION_REPORT_CTL%>?type=doc"
					target="_blank" class="btn btn-sm btn-info fw-semibold"> <i
					class="bi bi-file-earmark-word me-1"></i><%=ms.get("print.doc")%>
				</a> <a href="<%=ORSView.FACE_RECOGNITION_CTL%>"
					class="btn btn-sm btn-light text-primary fw-semibold"> <i
					class="bi bi-plus-circle me-1"></i> Add Face Recognition
				</a>
			</div>
		</div>
	</div>
	<form action="<%=ORSView.FACE_RECOGNITION_LIST_CTL%>" method="POST">
		...
		<div
			...
			<input type="text" name="name" class="form-control form-control-sm"
				style="max-width: 180px;" placeholder="Enter User Name"
				value="<%=ServletUtility.getParameter("name", request)%>"> <input
				type="text" name="code" class="form-control form-control-sm"
				style="max-width: 200px;" placeholder="Enter Face Code"
			...
			<button type="submit" name="operation" value="<%=BaseCtl.OP_DELETE%>"
				...
				onclick="return confirm('Delete this Face Recognition ?')">
