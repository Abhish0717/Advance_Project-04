	...
	<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
		...
		<div
			...
			<h5 class="mb-0 fw-bold">
				<i class="bi bi-building me-2"></i> <%=ms.get("college.list")%>
			</h5>
			<div class="d-flex gap-2">
				<a href="<%=ORSView.COLLEGE_REPORT_CTL%>" target="_blank"
					...
					class="bi bi-plus-circle me-1"></i> <%=ms.get("college.add")%>
				</a>
			</div>
		</div>
		...
		<form action="<%=ORSView.COLLEGE_LIST_CTL%>" method="POST">
			...
			<div
				...
				<input type="text" name="name" class="form-control form-control-sm"
					style="max-width: 200px;" placeholder="<%=ms.get("search.name")%>"
					value="<%=ServletUtility.getParameter("name", request)%>">
				<input type="text" name="city" class="form-control form-control-sm"
					style="max-width: 180px;" placeholder="<%=ms.get("search.city") %>"
				...
				<table class="table table-hover align-middle mb-0">
					<thead class="table-light">
						<tr>
							...
							<th><%=ms.get("address.title")%></th>
							<th><%=ms.get("state.title")%></th>
							<th><%=ms.get("city.title")%></th>
							<th><%=ms.get("phone.title")%></th>
